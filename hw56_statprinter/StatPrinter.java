...
public class StatPrinter {
    ...
    public boolean isLocalMode( int i )
    {
      ...
      && _frequency.get(i).compareTo(_frequency.get(i+1)) > 0
        && _frequency.get(i).compareTo(_frequency.get(i -1)) > 0);
    }
    ...
    public void printHistogram( int longestBar )
    {
      ...
      float ratio = (float)(longestBar/max);
      for (int x = 0; x <_frequency.size(); x++) {
      String stars = "";
        for (int s = 0; s < (int) _frequency.get(x) * ratio; s++) {
          stars += "*";
        }
